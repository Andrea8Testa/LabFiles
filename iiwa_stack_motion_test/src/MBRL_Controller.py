...
class jey_MBRL():
    ...
    def __init__(self, NN_ensemble_, ensemble_size_, prediction_horizon_, samples_num_):
        ...
        Wrench_sub = message_filters.Subscriber("/iiwa/state/CartesianWrench", WrenchStamped, queue_size =1,  buff_size = 2**20)
        JointPositionVelocity_sub = message_filters.Subscriber("/iiwa/state/JointPositionVelocity", JointPositionVelocity, queue_size =1, buff_size = 2**20)

        sync = message_filters.ApproximateTimeSynchronizer([Wrench_sub, JointPositionVelocity_sub], queue_size=1, slop = 0.1 )
        ...
        self.xn_train, self.yn_train_d, self.xn_test, self.yn_test_d, self.xy_norm = np.load('/home/jey/Training_data/Random_jey/Processed_data/Processed_Z_data/Prepared_random_New_Z.npy')
    ...
    def CEM_norm_p(self, x_initial_, action_dim_, time_horizon_, num_samples_, xy_norm_, NN_model_, num_ensembles_cem_):
        ...
        for _ in range(iteration_):
            ...
            for t in range(time_horizon_):
                ...
                state_action_norm_torch_ = torch.tensor(state_action_norm_, dtype=torch.float32, device="cuda:0")
                state_t_broadcasted_norm_torch_ = torch.tensor(state_t_broadcasted_norm_, dtype=torch.float32, device="cuda:0")
    ...
    def joint_states_callback(self, wrench, joint_position_velocity):
        ...
        self.joint_position_vector = np.array([joint_position_velocity.position.a1, joint_position_velocity.position.a2, joint_position_velocity.position.a3,
                                               joint_position_velocity.position.a4, joint_position_velocity.position.a5, joint_position_velocity.position.a6, joint_position_velocity.position.a7])

        self.joint_velocity_vector = np.array([joint_position_velocity.velocity.a1, joint_position_velocity.velocity.a2, joint_position_velocity.velocity.a3,
                                               joint_position_velocity.velocity.a4, joint_position_velocity.velocity.a5, joint_position_velocity.velocity.a6, joint_position_velocity.velocity.a7])
...
if __name__=='__main__':
    ...
    # Loading the Data
    xn_train, yn_train_d, xn_test, yn_test_d, xy_norm = np.load('/home/jey/Training_data/Random_jey/Processed_data/Processed_Z_data/Prepared_random_New_Z.npy')
    ...
    for i in range(num_ensembles):
        NN_delta_norm_cem_ensemble["NN" + str(i)].to("cuda:0")
