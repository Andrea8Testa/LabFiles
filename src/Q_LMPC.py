...
class Q_LMPC():
    ...
    def __init__(self, Device_, ensemble_NN_, actor_NN_, critic_NN_, ensemble_size_, prediction_horizon_, samples_num_):
    ...
    def CEM_norm_p(self, x_initial_, action_dim_, time_horizon_, num_samples_, xy_norm_, NN_model_, num_ensembles_cem_):
        ...
        for _ in range(iteration_):
            ...
            for t in range(time_horizon_):
                ...
                state_action_damping_norm_torch_ = torch.tensor(state_action_damping_norm_, dtype=torch.float32, device=self.Device)
    ...
    def Critic_train(self, state_force_train, action_train, damping_train, Actor_NN, Critic_NN, xy_norm, Cost_norm, Nh, learning_rate):
        ...
        actor_data_torch_ = torch.tensor(actor_eval_, dtype=torch.float32, device=self.Device)
        critic_data_torch_ = torch.tensor(critic_train_, dtype=torch.float32, device=self.Device)
        critic_data_torchp1_ = torch.tensor(critic_trainp1_, dtype=torch.float32, device=self.Device)
        ...
        for _ in range(iteration_):
            ...
            for t in range(time_horizon_):
                ...
                "fare somma di queste funzioni"
                 del Matrix_Z_u_; del Matrix_Z_D_;
                """
            ...
            """
            Cost_constraint_ = 100*np.ones((num_samples_,1))
...
if __name__=='__main__':
    ...
    myController = Q_LMPC(Device, model_approximator, actor, critic, num_ensembles, T, N)
    rospy.spin()
