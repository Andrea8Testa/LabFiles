...
class Q_LMPC():
    ...
    def __init__(self, NN_ensemble_, ensemble_size_, prediction_horizon_, samples_num_):
        ...
        cartesian_position_sub = rospy.Subscriber("/franka_ee_pose", Pose, queue_size = 1, buff_size = 10)
        cartesian_velocity_sub = rospy.Subscriber("/franka_ee_velocity", Twist, queue_size = 1, buff_size = 10)
        wrench_sub = rospy.Subscriber("/franka_ee_wrench", Wrench, queue_size = 1, buff_size = 10)
        ...
        self.u_pub = rospy.Publisher("/QLMPC_pose", geometry_msgs.msg.Pose, queue_size = 1)
        ...
        self.fh_mean = np.mean(self.Xtr[:,2:3])
        self.fh_std  = np.std(self.Xtr[:,2:3])
        ...
        self.z_record_norm = np.zeros(self.prediction_horizon)
        self.x_record_norm = np.zeros(self.prediction_horizon)
        self.fh_record_norm = np.zeros(self.prediction_horizon)
        self.u_record_norm = np.zeros(self.prediction_horizon)
        self.D_record_norm = np.zeros(self.prediction_horizon)
        self.iter_buffer_ = 0
    ...
    def CEM_norm_p(self, x_initial_, action_dim_, time_horizon_, num_samples_, xy_norm_, NN_model_, num_ensembles_cem_):
        ...
        for _ in range(iteration_):
            ...
            for t in range(time_horizon_):
                ...
                state_action_damping_norm_torch_ = torch.tensor(state_action_damping_norm_, dtype=torch.float32, device="cuda:0")
    ...
    def model_approximator_train(self, state_force_norm_train, u_norm_train, D_norm_train, NN_model, Nh_, learning_rate):
        ...
        NN_model_.to("cuda:0")  # putting the model into GPU (but not available now)
        ...
        # Forward propagation
        x_ = torch.tensor(x_data_, dtype= torch.float32, device="cuda:0")
        y_ = torch.tensor(y_data_, dtype= torch.float32, device="cuda:0")
    ...
    def Critic_train(self, state_force_train, action_train, damping_train, Actor_NN, Critic_NN, xy_norm, Cost_norm, Nh, learning_rate):
        ...
        Actor_NN_.to("cuda:0")  # putting the model into GPU (but not available now)
        Critic_NN_.to("cuda:0")
        ...
        actor_data_torch_ = torch.tensor(actor_eval_, dtype=torch.float32, device="cuda:0")
        critic_data_torch_ = torch.tensor(critic_train_, dtype=torch.float32, device="cuda:0")
        critic_data_torchp1_ = torch.tensor(critic_trainp1_, dtype=torch.float32, device="cuda:0")
    ...
    def ComputationUP(self, xy_norm, state_action_norm_torch_lie, model_approximator, num_ensembles = 5):
        ...
        for k in range(num_ensembles):
            model_approximator["NN"+str(k)].to("cuda:0")
    ...
    def Actor_train(self, state_force_train, Cost_norm, Actor_NN, Critic_NN, Model_NN, xy_norm, Nh, learning_rate):
        ...
        actor_input_torch_ = torch.tensor(state_force_trainp1_, dtype=torch.float32, device="cuda:0")
    ...
    def measurements_callback(self, pose, velocity, wrench):
        ...
        EXTERNAL_FORCES   = np.array([wrench.force.x, wrench.force.y, wrench.force.z])
        print("msg.Wrench: ", EXTERNAL_FORCES)
        
        CARTESIAN_POSE = np.array([pose.position.x, pose.position.y, pose.position.z, pose.orientation.x, pose.orientation.y,
                                   pose.orientation.z, pose.orientation.w])
        CARTESIAN_VEL = np.array([velocity.linear.x, velocity.linear.y, velocity.linear.z, velocity.angular.x, velocity.angular.y,
                                  velocity.angular.z]) 
        ...
        actor_input_norm = np.array([z_norm, x_norm, fh_norm])
        
        self.actor.to("cuda:0")
        self.actor.eval()
        # normalization of the input of the actor network
        actor_data_torch_ = torch.tensor(actor_input_norm, dtype=torch.float32, device="cuda:0")
        u_D_3_norm = self.actor.forward(actor_data_torch_)
        # denormalization
        ...
        self.u_record_norm[self.iter_buffer_] = u_D_3_norm[0,0]
        self.D_record_norm[self.iter_buffer_] = u_D_3_norm[0,1]
        self.z_record_norm[self.iter_buffer_] = z_norm
        self.x_record_norm[self.iter_buffer_] = x_norm
        self.fh_record_norm[self.iter_buffer_] = fh_norm
        
        u_message = geometry_msgs.msg.Pose()
        
        u_message.position.x = pose.position.x
        u_message.position.y = pose.position.y                   
        u_message.position.z = self.u_3
        u_message.orientation.x = pose.orientation.x                           
        u_message.orientation.y = pose.orientation.y            
        u_message.orientation.z = pose.orientation.z  
        u_message.orientation.w = pose.orientation.w  
        ...
        if (self.iter_buffer_ % self.prediction_horizon == 0):
            ...
            z_tra = self.z_record_norm.T
            x_tra = self.x_record_norm.T
            fh_record_tra = self.fh_record_norm.T
            state_norm_tra = np.append(z_tra, x_tra, axis=1)
            state_force_norm_tra = np.append(state_norm_tra, fh_record_tra, axis=1)
            u_record_tra = self.u_record_norm.T
            D_record_tra = self.D_record_norm.T
            print("x: ", self.x_record_norm)
            
            for n_rete in range(self.ensemble_size):
                self.model_approximator_train(state_force_norm_tra, u_record_tra, D_record_tra, self.model_approximator["NN" + str(n_rete)], self.prediction_horizon,
                                              learning_rate = 1e-3)
            
    
            print("CRITIC")
            self.Critic_train(state_force_norm_tra, u_record_tra, D_record_tra, self.actor, self.critic, self.xy_norm, self.Cost_norm, self.prediction_horizon, 
                         learning_rate = 5e-5)
            print("ACTOR")
            self.Actor_train(state_force_norm_tra, self.Cost_norm, self.actor, self.critic, self.model_approximator, self.xy_norm, self.prediction_horizon, learning_rate = 5e-3)
...
if __name__=='__main__':
    ...
    # Loading the Data
    training_dict = torch.load('tensors.pt')
    ...
    for i in range(num_ensembles):
        model_approximator["NN" + str(i)].to("cuda:0")
        model_approximator["NN" + str(i)].eval()
    
    print("QLMPC controller is starting")
    myController = Q_LMPC(model_approximator, num_ensembles, T, N)
    rospy.spin()
